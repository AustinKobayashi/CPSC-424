...
class Vertex {
    ...
    setNew(is_new) { this.is_new = is_new; }

    isNew() { return this.is_new; }
}
...
class HalfEdge {
    ...
    setIsSplit(is_split) { this.is_split = is_split; }
    isSplit() { return this.is_split; }
}
...
class Face {
    ...
    isTriangle () {
        let origin = this.he.getOrigin().getPos();

        if (!this.he.getNext())
            return false;

        if (!this.he.getNext().getNext())
            return false;

        if (!this.he.getNext().getNext().getNext())
            return false;

        return this.he.getNext().getNext().getNext().getOrigin().getPos().equal(origin);
    }
}
...
class Mesh {
    ...
    updateEdgeEnd (from, to, new_to) {

        let he = this.findEdge(from, to);
        if (!he)
            return;

        var old_key = String(from.getId()) + "," + String(to.getId());
        var new_key = String(from.getId()) + "," + String(new_to.getId());
        this.edgeMap.delete(old_key);
        this.edgeMap.set(new_key, he);

        // Associate edge with its twin, if it exists
        var t_he = this.findEdge(new_to, from);
        if (t_he === undefined) {}
        else {
            he.setTwin(t_he);
            t_he.setTwin(he);
    ...
    set_edge_count() { this.edge_count = this.edges.length; }

    set_face_count() { this.face_count = this.faces.length; }

    get_unsplit_edge() {
        if (this.edge_index >= this.edge_count)
            return undefined;

        while(this.edges[this.edge_index].isSplit()) {
            this.edge_index++;
            if (this.edge_index >= this.edge_count)
                return undefined;
        }

        return this.edges[this.edge_index++];
    }

    get_non_triangle_face() {
        if (this.face_index >= this.face_count)
            return undefined;

        while(this.faces[this.face_index].isTriangle()) {
            this.face_index++;
            if (this.face_index >= this.face_count)
                return undefined;
        }
        return this.faces[this.face_index];
    }
